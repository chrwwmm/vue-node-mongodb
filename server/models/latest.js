...
var LatestSchema = require('../schemas/latest') //拿到导出的数据集模块
var Latest = mongoose.model('Latest', LatestSchema) // 编译生成模型
var StoriesSchema = require('../schemas/stories') //拿到导出的数据集模块
var Stories = mongoose.model('Stories', StoriesSchema) // 编译生成模型
...
module.exports = {
	readAll: function(req, res, next) {
		Latest.find()
			.populate('top', 'imageUrl title')
			.populate('stories', 'imageUrl title')
			.sort({ _id: -1 }) //排序
...
//		Latest.create(doc,function(error){
//		    if(error) {
//		        console.log(error);
//		    } else {
//		        console.log('add ok');
...
//		Latest.findOne(del, function(err, result) {
//			if(err) {
//				return next(err);
//			}
//			if(!result) {
...
//				});
//			} else {
//				Latest.remove(del, function(err, result) {
//					if(err) {
//						return next(err);
//					} else {
//						return res.status(200).json({
...
//		Latest.findOne({_id: id},  function(err, result) {
//			if(err) {
//				return next(err);
//			}
//			if(!result) {
...
//					}
//				};
//				Latest.update(oldValue, newData, function(err, result) {
//					if(err) {
//						return next(err);
//					} else {
//						return res.status(200).json({
