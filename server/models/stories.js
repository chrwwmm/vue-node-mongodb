...
module.exports = {
	...
	addData: function(req, res, next) { //新增
		...
		Stories.create(doc,function(error){
		    if(error) {
		        console.log(error);
		    } else {
		        console.log('add ok');
	...
	delData: function(req, res, next) { //删除latest
		...
		Stories.findOne(del, function(err, result) {
			if(err) {
				return next(err);
			}
			if(!result) {
				...
				Stories.remove(del, function(err, result) {
					if(err) {
						return next(err);
					} else {
						return res.status(200).json({
	...
	modifyData: function(req, res, next) { //修改latest
		...
		Stories.findOne({_id: id},  function(err, result) {
			if(err) {
				return next(err);
			}
			if(!result) {
				...
				Stories.update(oldValue, newData, function(err, result) {
					if(err) {
						return next(err);
					} else {
						return res.status(200).json({
